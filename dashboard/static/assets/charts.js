function load_styles() {
    loadresource('/static/assets/charts.css', 'css', '')
}
load_styles()
function load_d3() {
    loadresource('https://cdn.jsdelivr.net/npm/d3@7', 'js', 'script_d3', load_plot)
}
function load_plot() {
...
function rendercharts() {
    for (let i = 0; i < charts.length; i++) {
        renderchart(charts[i].el, charts[i].data, charts[i].options)
    }
}
...
function renderchart(el, data, options) {
    const colours = ['#6493C5', '#0D3174']
    const div = document.getElementById(el)
    const keys = Object.keys((data[0].data ? data[0].data : data[0])[0])
    const type = options.type || 'bar'
    const xkey = options.xkey || keys[0]
    const ykey = options.ykey || keys[1]
    const zkey = options.zkey || null
    const limit = options.limit || 0
    const domain = (data[0].data || data[0]).map(obj => (obj[xkey]))
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        chartdata = chartdata.map(obj => ({ [xkey]: obj[xkey], [ykey]: obj[ykey], [zkey]: obj[zkey] || null }))
        //console.log(`chartdata: ${JSON.stringify(chartdata)}`)
...
function rasterize(el) {
    ...
    const svgs = document.getElementById(el).getElementsByTagName('svg')
