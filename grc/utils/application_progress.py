...
def save_progress():
    ...
    application_record = Application.query.filter_by(
        reference_number=session["reference_number"], email=session["email"]
    ).first()
    ...
    try:
        if "application" in session:
            ...
            if (
                ...
                and session["application"]["confirmation"]["overseasCheck"] == "No"
            ):
                session["application"]["medicalReports"][
                    "progress"
                ] = ListStatus.NOT_STARTED.name
                session["application"] = save_progress()
            elif (
                session["application"]["medicalReports"]["progress"]
                != ListStatus.CANNOT_START_YET.name
                and session["application"]["confirmation"]["overseasCheck"] == "Yes"
            ):
                session["application"]["medicalReports"][
                    "progress"
                ] = ListStatus.CANNOT_START_YET.name
            ...
            if (
                ...
                == ListStatus.CANNOT_START_YET.name
                and "marriageCivilPartnership"
                in session["application"]["personalDetails"]
                and (
                    ...
                    or (
                        ...
                        and (
                            ...
                            == "Yes"
                            or session["application"]["personalDetails"][
                                "marriageCivilPartnership"
                            ]["endedCheck"]
                            == "Yes"
                        )
                ...
                and (
                    ...
                    or (
                        ...
                        == "Neither"
                        and session["application"]["personalDetails"][
                            "marriageCivilPartnership"
                        ]["partnerDied"]
                        != "Yes"
                        and session["application"]["personalDetails"][
            ...
            ] = calculate_progress_status_display_name(
                ListStatus[session["application"]["statutoryDeclarations"]["progress"]]
            )

            # Submit and pay
            if (
                session["application"]["submitAndPay"]["progress"]
                == ListStatus.CANNOT_START_YET.name
                ...
                and (
                    session["application"]["nameChange"]["progress"]
                    == ListStatus.CANNOT_START_YET.name
                    or session["application"]["nameChange"]["progress"]
                    == ListStatus.COMPLETED.name
                )
                and (
                    session["application"]["marriageDocuments"]["progress"]
                    == ListStatus.CANNOT_START_YET.name
                    or session["application"]["marriageDocuments"]["progress"]
                    == ListStatus.COMPLETED.name
                )
            ):
                session["application"]["submitAndPay"][
                    "progress"
                ] = ListStatus.NOT_STARTED.name
                session["application"] = save_progress()
                ...
                "progress"
            ] != ListStatus.CANNOT_START_YET.name and not (
