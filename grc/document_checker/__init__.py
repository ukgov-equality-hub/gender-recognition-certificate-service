...
from grc.document_checker.forms import (
    PreviousNamesCheck,
    MarriageCivilPartnershipForm,
    StayTogetherForm,
    PartnerAgreesForm,
    PartnerDiedForm,
    InterimCheckForm,
    OverseasApprovedCheckForm,
    EmailForm,
    ValidateEmailForm,
)
...
@documentChecker.route(
    "/check-documents/personal-details/previous-names-check", methods=["GET", "POST"]
)
...
def partnershipDetails():
    ...
    if form.validate_on_submit():
        session["documentChecker"]["partnershipDetails"][
            "marriageCivilPartnership"
        ] = form.check.data
        session["documentChecker"] = session["documentChecker"]
...
@documentChecker.route(
    "/check-documents/partnership-details/stay-together", methods=["GET", "POST"]
)
def stayTogether():
...
@documentChecker.route(
    "/check-documents/partnership-details/partner-agrees", methods=["GET", "POST"]
)
def partnerAgrees():
...
@documentChecker.route(
    "/check-documents/partnership-details/interim-check", methods=["GET", "POST"]
)
def interimCheck():
...
@documentChecker.route(
    "/check-documents/partnership-details/partner-died", methods=["GET", "POST"]
)
def partnerDied():
...
@documentChecker.route(
    "/check-documents/partnership-details/ended-check", methods=["GET", "POST"]
)
def endedCheck():
...
@documentChecker.route(
    "/check-documents/overseas-approved-check", methods=["GET", "POST"]
)
def overseas_approved_check():
    form = OverseasApprovedCheckForm()

    if form.validate_on_submit():
        session["documentChecker"]["confirmation"][
    ...
    if (
        session["documentChecker"]["partnershipDetails"]["marriageCivilPartnership"]
        == "Neither"
    ):
    ...
    return render_template(
        "document-checker/overseas-approved-check.html", form=form, back=back
    )
...
def email():
    ...
    if (
        "documentChecker" not in session
        or session["documentChecker"]["confirmation"]["overseasApprovedCheck"] is ""
    ):
    ...
    return render_template("document-checker/email-address.html", form=form)
...
def emailConfirmation():
    ...
    return render_template(
        "document-checker/security-code.html", form=form, email=session["email"]
    )
