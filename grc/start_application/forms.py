...
class OverseasCheckForm(FlaskForm):
    check = RadioField('check', choices=[('Yes'),('No')], validators=[DataRequired(message='Select if you ever been issued a Gender Recognition Certificate')])
    submit = SubmitField('Continue')
...
class DeclerationForm(FlaskForm):
    check = BooleanField('check', validators=[DataRequired(message='Confirm that you meet requirements')])
    submit = SubmitField('Continue')